...
# ទទួលបាន URL ពី Cloud Run និង PORT
...
def is_ffmpeg_installed():
    return True # Cloud Run នឹងដំឡើងវាតាមរយៈ Dockerfile រួចហើយ
...
def main() -> None:
    ...
    if not WEBHOOK_URL:
        print("!!! កំហុស៖ CLOUD_RUN_URL មិនត្រូវបានកំណត់។ ត្រូវប្រាកដថាបានបន្ថែមវានៅក្នុង Cloud Run Environment Variables។")
        # ទោះគ្មាន URL ក៏ឱ្យ Cloud Run ដកដង្ហើមសិន ដើម្បីកុំឱ្យ Crash ពេលទើប Deploy ថ្មីៗ
        print("Bot ផ្អាករង់ចាំ Webhook URL...")
        sys.exit(1)
